...
if __name__ == '__main__':
    ...
    for count, recipe in enumerate(system_variables.initial_recipes):
        ...
        recipe_file.close()
    for count, data in enumerate(system_variables.initial_data):
        data_file = open(system_variables.initial_data_path + '\\data_' + str(count) + system_variables.data_extension, 'w')
        data_file.write(data)
        data_file.close()
    for count, pattern in enumerate(system_variables.initial_patterns):
        pattern_file = open(system_variables.pattern_path + '\\pattern_' + str(count) + system_variables.pattern_extension, 'w')
    ...
    meta_process_list = [
        directory_monitor(system_variables.pattern_path, pattern_monitor_to_handler.writer()),
        pattern_handler(pattern_monitor_to_handler.reader(), pattern_handler_to_task_generator.writer()),
        directory_monitor(system_variables.recipe_path, recipe_monitor_to_handler.writer()),
        recipe_handler(recipe_monitor_to_handler.reader(), recipe_handler_to_task_generator.writer()),
        directory_monitor(system_variables.data_path, data_monitor_to_handler.writer()),
